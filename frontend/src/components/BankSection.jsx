...
const BankContent = ({content,setContent,selectedBank,setSelectedBank,bank}) => {
  ...
  async function deleteBank(){
    try {
      setIsLoading(true)
      const response = await fetch(`https://secureguard-production.up.railway.app/api/bank/${selectedBank}`,{
        method: "DELETE",
        headers: {
      ...
      cache.match(`https://secureguard-production.up.railway.app/api/bank/${id}`).then((cachedResponse) => {
        if (cachedResponse) {
          cachedResponse.json().then((cachedData) => {
            cache.put(
              `https://secureguard-production.up.railway.app/api/bank/${id}`,
